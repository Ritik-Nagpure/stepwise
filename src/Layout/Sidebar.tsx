...
import { faBell, faSun, faMoon, faHome, faUser, faCog, faDashboard } from '@fortawesome/free-solid-svg-icons';
...
const Sidebar = ({ isDark, toggleMode, expandSidebar }: SidebarProps) => {
  ...
  return (
    <div
      className={`flex flex-col border-4 border-gray-400 bg-[#cecdcd] shadow-gray-950 rounded-xl h-full py-4 px-2 mx-0.5 shadow-2xl transition-all duration-300 ${expandSidebar ? 'w-56' : 'w-16'}`}    >
      ...
      <div className="flex flex-col gap-4">
        ...
        {menuItems.map((item, index) => (
          <div key={index} className="flex items-center hover:bg-amber-600 p-2 rounded-lg cursor-pointer" >
            <Link to={`/stepwise/${item.label}`}>
